...
__device__ void logistic_step(Real *t, Real *tnew, int len_1){
/*
    int i;
    for(i=0;i<=len_1;i++)
       t[i]=FUN(t[i]);
...
__device__ void rand_need_more(Real t[RAND_BUF_LEN],Real tbuf[RAND_BUF_LEN]){
...
__device__ float rand_next_scatlen(Real t[RAND_BUF_LEN]){
    Real ran=rand_uniform01(t[0]);
    if(ran==0.f) ran=rand_uniform01(t[1]);
    return ((ran==0.f)?LOG_MT_MAX:(-logf(ran)));
}
// generate [0,1] random number for the next arimuthal angle
__device__ float rand_next_aangle(Real t[RAND_BUF_LEN]){
    return rand_uniform01(t[2]);
}
// generate random number for the next zenith angle
__device__ float rand_next_zangle(Real t[RAND_BUF_LEN]){
    return rand_uniform01(t[4]);
}

// generate random number for the next zenith angle
__device__ float rand_next_reflect(Real t[RAND_BUF_LEN]){
    return rand_uniform01(t[3]);
}
// generate random number for the next zenith angle
__device__ float rand_do_roulette(Real t[RAND_BUF_LEN]){
