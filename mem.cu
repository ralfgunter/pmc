...
void init_mem(ExecConfig conf, Simulation *sim, GPUMemory *gmem)
{
    ...
    h_tissueProp = (float3 *) malloc((sim->tiss.num + 1) * sizeof(float3));
    for (int i = 0; i <= sim->tiss.num; i++)
    {
        h_tissueProp[i].x = sim->tiss.musr[i];
        h_tissueProp[i].y = sim->tiss.mua[i];
        h_tissueProp[i].z = sim->tiss.g[i];
    }
    ...
    cudaMemcpyToSymbol("detLoc", h_detLoc, sim->det.num * sizeof(int3), 0, cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol("tissueProp", h_tissueProp, (sim->tiss.num + 1) * sizeof(float3), 0, cudaMemcpyHostToDevice);
